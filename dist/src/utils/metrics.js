    ...
    if (!operationName) {
        try {
            ...
            saveMetricsConsume(metric).then((res) => {
                const { data } = res;
                if (!data) {
                    const { status, statusText, data } = (res === null || res === void 0 ? void 0 : res.response) || {};
                    console.log("*ERROR*", "Failed to save metric: [", status, " - ", statusText, " - ", data, "]");
                }
            });
        }
        catch (err) {
            console.log("*ERROR*", "Failed to save metric: [", err === null || err === void 0 ? void 0 : err.message, "]");
        }
    }
};
const validateApplication = (req) => {
    ...
    if (!consumer || !requestId) {
        console.log("*ERROR*", "No consumer and trace heades: [", JSON.stringify(req.headers), "]");
